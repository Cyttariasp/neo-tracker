...
export default function Transactions() {
  ...
  const handleAdd = (tx) => {
    ...
    // si no trae kind (compatibilidad con datos antiguos), asumir "expense"
    const txNorm = { kind: "expense", ...tx, id };
    setTransactions((prev) => [txNorm, ...prev]);
    showToast(txNorm.kind === "income" ? "Ingreso agregado" : "Gasto agregado");
  };
  ...
  const view = useMemo(() => {
    ...
    // total solo de gastos
    const total = sorted.reduce((acc, t) => acc + (t.kind === "income" ? 0 : (Number(t.amount) || 0)), 0);
  ...
  // Agrupar por mes lo filtrado/ordenado
  const groupedByMonth = useMemo(() => {
    ...
    return Array.from(groups.entries()); // mantiene el orden de view.items
  }, [view.items]);

  const prettyMonth = (ym) => (ym === "sin-fecha" ? "Sin fecha" : monthLabel(ym));
      ...
      <div className="container d-flex flex-column gap-2 mb-2">
        ...
        {hasActiveFilters && (
          <div className="d-flex align-items-center gap-2">
            {filters.month !== "all" && (
              <span className="badge rounded-pill text-bg-light border">{monthLabel(filters.month)}</span>
            )}
      ...
      <div className="tx-sticky">
        <div className="container d-flex justify-content-between align-items-end py-2">
          ...
          <span className="badge text-success-emphasis bg-success-subtle border border-success-subtle rounded-pill">
            Total (gastos): {formatCurrency(view.total)}
          </span>
        </div>
      </div>
      ...
      {view.items.length === 0 ? (
        <div className="container">
          <div className="card card-surface shadow-soft p-4">
            <p className="mb-0 text-muted">
              No hay resultados con los filtros actuales. Cambia los filtros o agrega un movimiento con <strong>➕</strong>.
            </p>
          </div>
        </div>
      ) : (
        <div className="w-100">
          {groupedByMonth.map(([ym, items]) => (
            <section key={ym}>
              {/* Encabezado de mes con total del mes (sólo gastos) */}
              {(() => {
                const monthTotal = items.reduce((acc, t) => acc + (t.kind === "income" ? 0 : (Number(t.amount) || 0)), 0);
                return (
                  <div
                    ...
                    <li key={tx.id} className="tx-item">
                      ...
                      <div className="tx-body">
                        <div className="tx-title">
                          ...
                          {isIncome && <span className="ms-2 badge text-bg-success">Ingreso</span>}
                        </div>
                      ...
                      <div className="tx-actions">
                        <span className={`badge ${isIncome ? "bg-success" : "bg-primary"} rounded-pill`}>
